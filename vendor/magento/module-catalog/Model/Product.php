    ...
    ProductInterface
{
    ...
    public function getcustomnamecategory(){
        $cats = $this->removecategories([23,24,40]);                            
        $cat = "";
    ...
    public function removecategories($catquitar){
        $cats = $this->getCategoryIds();
        foreach ($catquitar as $catq) {
            $buscarquitar = array_search($catq, $cats);  
            if ($buscarquitar){
                unset($cats[$buscarquitar]);
            }        
        }
        return $cats;
    }

    public function getsocialshare($url){      
        ...
        $html = '
        <div class="sharesocialnetworks">
            <a href="http://www.facebook.com/sharer.php?u='.$url.'&picture=https://kalea.4amsaatchi.com/resources/BA1443/BA1443.jpg" target="_blank">
              <img src="'.$baseurl.'pub/static/frontend/Magento/luma/es_MX/images/facebook.png" alt="Facebook" />
            </a>            
            <!--<a href="https://twitter.com/share?url='.$url.'" target="_blank">
                <img src="'.$baseurl.'pub/static/frontend/Magento/luma/es_MX/images/twitter.png" alt="Twitter" />
            </a>-->
            <a href="http://pinterest.com/pin/create/button/?url='.$url.'&media=https://simplesharebuttons.com/images/somacro/twitter.png" class="pin-it-button" count-layout="horizontal" target="_blank">
                <img border="0" src="'.$baseurl.'pub/static/frontend/Magento/luma/es_MX/images/pinterest.png" title="Pin It" />
            </a>
        </div> ';
        return $html; 
    }

    public function getImagenKalea(){        
        $sku = $this->getSku();
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $storeManager = $objectManager->get('\Magento\Store\Model\StoreManagerInterface');
        $baseurl = $storeManager->getStore()->getBaseUrl();
        $directorio = "/var/www/html/resources/".$sku."/";
        $uridir = $baseurl."resources/".$sku."/";
        $img = $baseurl."pub/static/frontend/Magento/luma/es_MX/Magento_Catalog/images/product/placeholder/image.jpg";
        $files = array();
        if (is_dir($directorio)){ //Comprobar de que exista la carpeta
            $files = glob($directorio . "*.{jpg,png,gif}", GLOB_BRACE);            
            if ($files){
                sort($files);
                $img = $uridir.basename(array_shift($files));                
            }            
        } 
        return $img;
    }

    public function getImageKaleaHtml(){
        return '<div class="product-image-container" style="width:600px;">
                    <div class="product-image-wrapper" style="padding-bottom: 100%;">
                         
                        <img class="product-image-photo" src="'.$this->getImagenKalea().'" width="135" height="135" alt="'.$this->getName().'"></div>
                </div>';
    }
