...
class Gallery extends \Magento\Catalog\Block\Product\View\AbstractView
{
    ...
    /* Extraer imagenes de directorio */ 
    public function getGalleryImagesKalea()
    {

        $product = $this->getProduct();
        $sku = $product->getSku();
        $directorio = "/var/www/html/resources/".$sku."/";
        $uridir = $this->getBaseUrl()."resources/".$sku."/";
        $arrayimgas = array();
        ...
        if (is_dir($directorio)){ //Comprobar de que exista la carpeta
            ...
            if ($files){
                ...
                /*$lastElement = $uridir.basename(end($files));
                
                array_push($arrayimgas, array("caption"=>"","full"=>$lastElement, "isMain"=>true, "position"=>1, "thumb"=>$lastElement, "type"=>"image", "videoUrl"=>false));
                array_pop($files);*/
                $cont=1;
                $isMain= false;
                foreach ($files as $img) {  
                    $rutimg = $uridir.basename($img);
                    if ($cont == 1){
                        $isMain = true;
                    }else{
                        $isMain= false;
                    }
                    $arrayim = array("caption"=>"","full"=>$rutimg, "isMain"=>$isMain, "position"=>$cont, "thumb"=>$rutimg, "type"=>"image", "videoUrl"=>false);
                    array_push($arrayimgas, $arrayim);
                    $cont = $cont + 1; 
                }
                return json_encode($arrayimgas); 
            }else {
                return -99;
            }
            
        } else {
            //Si no encontro directorio
            $img = $this->getBaseUrl()."pub/static/frontend/Magento/luma/es_MX/Magento_Catalog/images/product/placeholder/image.jpg";            
            array_push($arrayimgas, array("caption"=>"","full"=>$img, "isMain"=>true, "position"=>1, "thumb"=>$img, "type"=>"image", "videoUrl"=>false));
            return json_encode($arrayimgas);
        }
