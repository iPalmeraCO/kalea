...
class Cart extends DataObject implements CartInterface
{
    ...
    public function addProduct($productInfo, $requestInfo = null)
    {
        ...
        $productaux = $this->_getProduct($productInfo);
        $a = new \Inchoo\Helloworld\Model\ApiKalea();
        $exis =  $a->consulta_existencias($productaux->getSku());

        if ($exis == -1 ){
            throw new \Magento\Framework\Exception\LocalizedException(__('The product does not exist.'));
        }
        $exis = $exis["descripcion"]["total_inmediatas"];

        $productaux->setStockData(
                    array(
        ...
        /* Usuario logueado */
         ...
         if ($no_transa_mov == -1) {
            ...
            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $request2 = $objectManager->get('Magento\Framework\App\Request\Http');            
             //Url del producto a agregar
            $urlInterface = \Magento\Framework\App\ObjectManager::getInstance()->get('Magento\Framework\UrlInterface');                        
            $login_url = $urlInterface->getUrl('customer/account/login',array('referer'=>base64_encode($request2->getServer('HTTP_REFERER'))));  
            $this->_checkoutSession->setRedirectUrl($login_url);
            //Almacenar producto que se estaba intentando añadir y la cantidad
            $this->_checkoutSession->setProductinsert(base64_encode($product->getId()));
            $this->_checkoutSession->setCantidadinsert(base64_encode($request->getQty()));
    ...
    public function truncate()
    {
        ...
        /*$a = new \Inchoo\Helloworld\Model\ApiKalea();      
        $no_transa_mov = self::get_no_transa_mov();        
    ...
    public function eliminar(){
        $this->getQuote()->removeAllItemsMagento();
        return $this;
    }
    ...
    {
        throw new \Magento\Framework\Exception\LocalizedException(__('Tes.'));
        try {
            $item = $this->getQuote()->getItemById($itemId);
            if (!$item) {
...
/*
    public function get_no_transa_mov(){
      $a = new ApiKalea(); 
      $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $customerSession = $objectManager->get('Magento\Customer\Model\Session');
        if($customerSession->isLoggedIn()) {                
            ...
            if ($customer->getData('cotizacion') != "") {
                $no_transa_mov = $customer->getData('cotizacion'); 
            }else {                
                $nombre = $customer->getFirstname()." ".$customer->getLastname();
                 $cotizacion = $a->crear_cotizacion($customer->getData('idkalea'),$nombre,"0000");
                  if ($cotizacion["respuesta"]){
                    $no_transa_mov = $cotizacion["descripcion"]["cotizacion"]["no_transa_mov"];  
                    $customerData = $customer->getDataModel();
                    $customerData->setCustomAttribute('cotizacion', $no_transa_mov);
        ...
        return $no_transa_mov;
    }  */


    public function checkcotizacion(){        
        $retorno = true;
        $no_transa_mov = $this->getQuote()->get_no_cotizacion();             
        if (trim($no_transa_mov) == ""){
            $this->getQuote()->delete();
            throw new \Magento\Framework\Exception\LocalizedException(__('Error al cargar cotización.'));     
        }
         if ($no_transa_mov == -1) {
            $this->_checkoutSession->setRedirectUrl("/customer/account/login");    
            throw new \Magento\Framework\Exception\LocalizedException(__('Debe estar logueado para poder continuar.')); 
